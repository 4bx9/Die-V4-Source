	...
	public void logout()
        ...
        public void enable2fa(string code = null)
        {
            ...
            if (json.success)
            {
                if (code == null) // First time enabling 2FA, no code provided
                {
                    ...
                    MessageBox.Show($"Your 2FA Secret Code has been copied to your clipboard! \n\n: {json.twoFactor.SecretCode}", "2FA Secret");

                }
                else // Code provided by the user
                {
                    MessageBox.Show("2FA has been successfully enabled!", "2FA Setup");
                }
            }
            else
            {
        ...
        private static void sigCheck(string resp, WebHeaderCollection headers, string type)
        {
            ...
            try
            {
              ...
              //  bool signatureValid =  KeyAuth.encryption.CheckValid(byteSig, byteBody, byteKey); // the ... dots in the console are from this function!
               // if (!signatureValid)
                {
